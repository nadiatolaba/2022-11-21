...
function printNumber(number){
    if(operator===false) {
        if(value1==="0") {                 
            operationDisplay.innerHTML=number;  
            value1=number;                           
            if(number==="."){                 
...
function mathOperation(operatorSimbol){
   ...
   if(operator===false/* && aux!=="*" && aux!=="/" && aux!=="+" && aux!=="-" && aux!=="." && aux!=="%"*/){
    ...
    operation=operatorSimbol;
}
function equal(){
    switch(operation) {
        case "+":
            oper(Number(value1), Number(value2) , suma);
            break;
        case "*":
            oper(Number(value1), Number(value2) , multiplicacion);
            break;
        case "-":
            oper(Number(value1), Number(value2) , resta);
            break;
        case "/":
            oper(Number(value1), Number(value2) , division);
            break;
    }
}
